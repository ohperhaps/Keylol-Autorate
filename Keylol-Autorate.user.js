...
const HideAutorate = false; //显示体力冷却时间是是否隐藏Autorate true:hh:mm:ss；false:Autorate hh:mm:ss
    ...
    function AutoTimer() { //自动加体力
        ...
        if (Cooldown <1) { //判断体力冷却是否结束
            ...
            if(HideAutorate == false){ //显示体力冷却时间
                $('#autoRate').text('Autorate '+check(Hour)+':'+check(Minute)+':'+check(Second))
            }
            else {
                $('#autoRate').text(check(Hour)+':'+check(Minute)+':'+check(Second))
            }
        }
    }
    views()
